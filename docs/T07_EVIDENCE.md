...
화면(브라우저) 값과의 대조는 처음엔 실제 사용자 계정 로그인 자격 증명이 없어 수행하지 못했다 — 대신 **SEE 화면이 호출하는 것과 동일한 함수를 동일한 DB 값으로 직접 실행**해 코드 결과와 손계산을 대조했다(위 표). 코드와 손계산이 완전히 일치하므로, 집계 로직 자체에는 오차가 없음을 확인했다.

이후 사용자가 실제 로그인 화면 캡처(`image/run/see.png`)를 제공해, 진짜 SEE 화면 값과도 대조할 수 있었다 — 4부 참고. 캡처 시점에 14번째 할 일(id 177, TODO, 180분)이 새로 추가돼 있어 예상 시간·차이 값이 위 표(13건 기준)와 180분만큼 다르지만, 실제 시간(1098분)과 막힘 건수(4건)는 정확히 일치하고, 예상 시간 차이도 새 task의 estimatedMinutes(180분)로 완전히 설명된다 — 즉 화면 값도 코드 계산과 어긋나지 않는다.
...
`image/run/`에 사용자가 직접 캡처한 배포/로컬 화면 6장. 로그인 자격 증명 없이는 확인할 수 없었던 "실제 화면"을 이 캡처로 보완한다.

| 파일 | 화면 | 확인 내용 |
|---|---|---|
| landing.png | `/` 비로그인 첫 화면 | "OBSERVATORY · LOCKED" 잠금 게이트만 보이고 실제 계획/할 일 등 자료는 전혀 없음. "로그인하고 궤도 열기" / "처음이라면 회원가입" 진입점만 존재 — CLAUDE.md 4.2·11의 "미인증 상태에서 자료 비노출" 요건과 일치 |
| login.png | `/login` 로그인 탭 | 이메일/비밀번호 입력란 + "Google로 계속하기" 버튼. 비밀번호 입력란은 `type="password"`로 마스킹 표시 |
| join.png | `/login` 회원가입 탭 | 이름/이메일/비밀번호(8자 이상 안내)/비밀번호 확인 입력란 + Google 가입 버튼. 로그인 탭과 같은 화면에서 탭 전환(`AuthScreen.tsx`)으로 구현됨 |
| main.png | `/dashboard` 로그인 후 | 실사용자(이슬기)의 실제 계획 "정보처리기사 실기 공부" 별자리, 우측 상단 "PRIVATE · MY VIEW", TASKS 패널, DO 최근 실행 기록(09-22 09:00~10:00 포함 5건), SEE 예상 대 실제(EST 30h30m / ACTUAL 18h18m / DIFF -12h12m) — 로그인해야만 보이는 화면이며 우측 상단 nav에 PLAN/DO/SEE/MY/LOGOUT이 모두 노출됨(비로그인 시 HOME만 노출되는 것과 대조) |
| see.png | `/see` 돌아보기 | 계획된 할일 14건 · 완료 13건 · 지연 0건 · 막힘 4건 · 예상 30시간30분 · 실제 18시간18분 · 차이 -12시간12분. 아래 §화면-코드 대조표 참고 |
| complite.png | `/tasks` 전체 할 일 목록 | plan 3·48의 할 일이 검색/상태/우선순위/태그/정렬 필터와 함께 나열되며, 완료 처리된 항목에 "★완료" 배지가 붙어 있음 |

#### landing.png — `/` 비로그인 첫 화면
![비로그인 첫 화면: OBSERVATORY LOCKED 게이트](../image/run/landing.png)

#### login.png — `/login` 로그인 탭
![로그인 탭: 이메일/비밀번호 + Google로 계속하기](../image/run/login.png)

#### join.png — `/login` 회원가입 탭
![회원가입 탭: 이름/이메일/비밀번호/비밀번호 확인](../image/run/join.png)

#### main.png — `/dashboard` 로그인 후
![로그인 후 대시보드: 실제 계획 별자리, TASKS, DO, SEE 패널](../image/run/main.png)

#### see.png — `/see` 돌아보기
![SEE 화면: 계획 14건, 완료 13건, 예상 30h30m, 실제 18h18m, 차이 -12h12m](../image/run/see.png)

#### complite.png — `/tasks` 전체 할 일 목록
![전체 할 일 목록: 검색/상태/우선순위/태그/정렬 필터와 완료 배지](../image/run/complite.png)

### 화면(see.png) ↔ 코드 계산 대조표

캡처 시점 plan 3는 할 일 14건(13 DONE + 신규 1건 TODO, id 177 "모의고사 한 회차 풀고 오답노트 하기", 180분)으로 §2-3 검산 때보다 1건 늘어 있다. 그 차이만 반영하면 화면 값과 코드 계산이 정확히 일치한다.

| 항목 | see.png 화면 값 | 코드 재계산(현재 14건 기준) | 일치 |
|---|---|---|---|
| 계획된 할 일 | 14건 | 14건 (13 DONE + 1 TODO) | ✅ |
| 완료 | 13건 | 13건 | ✅ |
| 지연 | 0건 | 0건 | ✅ |
| 막힘 | 4건 | 4건 (§2-3과 동일 — 신규 task는 workLog 없음) | ✅ |
| 예상 시간 | 30시간 30분(1830분) | 1650(§2-3의 13건) + 180(신규 task) = **1830분** | ✅ |
| 실제 시간 | 18시간 18분(1098분) | 1098분 (§2-3과 동일 — 신규 task는 아직 실행 기록 없음) | ✅ |
| 차이 | -12시간 12분(-732분) | 1098-1830 = **-732분** | ✅ |

7개 항목 전부 일치. §2-3에서 "화면 대조는 로그인 정보가 없어 코드 직접 실행으로 대체했다"고 밝혔던 제약이, 사용자가 제공한 실제 화면 캡처로 완전히 해소되었다 — 코드 계산과 실제 브라우저 렌더링 값이 다르지 않음을 최종 확인했다.
